...
def initialize_manager():
    ...
    if st.session_state.manager is None:
        with st.spinner("Initializing system..."):
            try:
                # Check if ChromaDB directories exist but might be corrupted
                if os.path.exists(Config.CHROMA_PERSIST_DIRECTORY):
                    # Check if it's empty or corrupted
                    chroma_files = os.listdir(Config.CHROMA_PERSIST_DIRECTORY)
                    ...
                    try:
                        ...
                        # If Chroma fails, it might be corrupted - recreate it
                        if "tenant" in str(chroma_error).lower() or "connect" in str(chroma_error).lower():
                            st.warning("⚠️ Detected corrupted database. Recreating...")
                            
                            # More thorough cleanup with error handling
                            import time
                            max_retries = 3
                            for attempt in range(max_retries):
                                try:
                                    ...
                                    if attempt < max_retries - 1:
                                        ...
                                        st.warning(f"Could not fully remove directory: {rm_error}")
                            
                            # Recreate directory
                            os.makedirs(Config.CHROMA_PERSIST_DIRECTORY, exist_ok=True)
                            
                            # Also recreate knowledge base if needed
                            ...
                            try:
                                st.session_state.manager = TestCaseManager()
                                st.success(" System initialized with fresh database!")
                            except Exception as retry_error:
                                st.error(f"Failed to reinitialize: {retry_error}")
                                st.warning("⚠️ **Please restart the Streamlit server to fully reset the database.**")
                                st.markdown("""
                                **Steps to restart:**
                ...
                st.info("💡 **Solution**: Please restart the Streamlit server to fully reset the database connection.")
                st.code("Press Ctrl+C in the terminal and run: streamlit run ui/app.py")
                return False
    return True
...
def main():
    ...
    with st.sidebar:
        ...
        if not hasattr(st.session_state, 'manager') or st.session_state.manager is None:
    ...
    with tabs[0]:
        ...
        # Use default suite name
        import_suite_name = "default"
        ...
        if uploaded_file is not None:
            ...
            if file_ext in ['xlsx', 'xls']:
                ...
                **Excel Format Requirements:**
                ...
                - **Optional columns**: Business Rule, Preconditions, Postconditions, Tags, Priority, Test Type, ID
                """)
            else:
                st.markdown("""
                            ...
                            for i, tc in enumerate(result['test_cases'][:10], 1):
                                with st.expander(f"{i}. {tc.title}"):
                                    ...
                                    with col1:
                                        ...
                                        st.markdown(f"**Business Rule:** {tc.business_rule}")
                                    with col2:
    ...
    with tabs[1]:
        ...
        if input_method == "Text Input":
            ...
            with col1:
                auto_apply = st.checkbox("Auto-apply decisions", value=False)
            
            if st.button(" Generate & Analyze Test Cases", type="primary"):
                if requirement_text:
                    with st.spinner("Processing requirement..."):
                        try:
                            ...
                            # Trigger rerun to update statistics in sidebar
                            st.rerun()
                        except Exception as e:
                            st.error(f" Error: {e}")
                else:
            ...
            col1, col2 = st.columns([1, 4])
            with col1:
                auto_apply = st.checkbox("Auto-apply decisions", value=False, key="auto_apply_form")
            
            if st.button(" Generate & Analyze Test Cases", type="primary", key="process_story"):
            ...
            for i, result in enumerate(st.session_state.results['results'], 1):
                ...
                with st.expander(f"**{i}. {test_case.title}** - {comparison.decision.value.upper()}", expanded=True):
                    ...
                    with col1:
                        ...
                        st.markdown(f"- **Business Rule:** {test_case.business_rule}")
                    ...
                    with col2:
                        ...
                        if test_case.boundary_conditions:
                            st.markdown("**Boundary Conditions:**")
                            for bc in test_case.boundary_conditions:
                                st.markdown(f"  - {bc}")
                    
                    # Manual apply button if not auto-applied
                    if not st.session_state.results.get('actions_taken'):
                        if st.button(f" Apply Decision", key=f"apply_{i}"):
                            action = st.session_state.manager.apply_decision(
                                test_case,
                                comparison,
                                st.session_state.suite_name,
                                user_approved=True
