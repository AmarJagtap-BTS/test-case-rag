...
def validate_test_type(test_type: str) -> str:
    ...
    Accepts various test types including:
    - Functional, Integration, E2E, API, Security, Performance, UI, Regression, Smoke, etc.
    
    Args:
        test_type: Test type to validate
        
    Returns:
        Normalized test type string, defaults to 'Functional' if empty
    """
    if not test_type or not test_type.strip():
        return "Functional"
    
    # Normalize: strip whitespace and convert to title case
    normalized = test_type.strip()
    
    # Return as-is (allow any valid test type)
    # Common types: Functional, Integration, E2E, API, Security, Performance, 
...
def export_to_excel(test_cases: List[TestCase], output_path: str):
    ...
    for tc in test_cases:
        ...
        data.append({
            ...
            "Business Rule": tc.business_rule,
            ...
            "Boundary Conditions": ", ".join(tc.boundary_conditions),
            "Side Effects": ", ".join(tc.side_effects),
            "Version": tc.version,
...
def parse_test_case_json(json_data: Dict[str, Any]) -> TestCase:
    ...
    if not description:
        if title:
            description = f"Test case to verify: {title}"
        else:
            description = "Test case to verify functional requirement"
    
    # Validate test type - must be Positive or Negative
    test_type = validate_test_type(json_data.get("test_type", ""))
    
    # Ensure expected_outcome is always a string
    expected_outcome = json_data.get("expected_outcome", "")
    if isinstance(expected_outcome, dict):
        # If it's a dict, convert to a readable string
        expected_outcome = "; ".join([f"{k}: {v}" for k, v in expected_outcome.items()])
    elif not isinstance(expected_outcome, str):
        expected_outcome = str(expected_outcome)
    
    return TestCase(
        id=test_id,
        title=title if title else "Untitled Test Case",
    ...
    for row_idx, row in df.iterrows():
        try:
            ...
            if pd.isna(description) or not description.strip() or description == 'nan':
                ...
                description = f"Test case to verify: {title}" if title != 'Untitled Test' else "Test case to verify functional requirement"
