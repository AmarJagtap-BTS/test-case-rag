                ...
                "test_cases": [
                    {
                        ...
                        "business_rule": "Backend",
                        "preconditions": [
                            "User accessed registration page via browser", 
                            "No existing account registered with email test@example.com",
                            "Registration form fully loaded and displayed",
                            "Email service configured and operational",
                            "Database connection active and accepting writes"
                        ],
                        "test_steps": [
                ...
                "test_cases": [
                    {
                        ...
                        "business_rule": "API",
                        "preconditions": [
                            "API service running on port 8080 and responding to health checks", 
                            "Test user account exists in database with ID=123", 
                            "Valid JWT auth token generated and not expired",
                            "Network connectivity established between client and server"
                        ],
                        "test_steps": [
                ...
                "test_cases": [
                    {
                        ...
                        "business_rule": "Backend",
                        "preconditions": [
                            "User logged in with active session (expires in 30 minutes)", 
                            "Shopping cart contains 3 items with total value $127.50", 
                            "Payment gateway (Stripe) integrated and operational", 
                            "Shipping address 'John Doe, 123 Main St' saved in user profile",
                            "Sufficient inventory available for all cart items"
                        ],
                        "test_steps": [
...
3. business_rule (string): MUST be a simple category (1-2 words max)
   Use ONLY: Backend, Frontend, API, Database, Integration, UI, Business Logic, Network, Security, Performance
    CORRECT: "Backend" or "Frontend" or "API"
    WRONG: "System must validate user input and process requests"
4. preconditions (array): 5-8 moderately detailed items (50-150 chars each)
   Include key details like roles, formats, file specs, but stay concise
    "User logged in with 'Admin' role and verified email"
    "Excel file (.xlsx format, 3.2MB) prepared with 75 records"
    Too basic: "User logged in"
    Ultra-detailed: "User 'John Doe' (ID: USR_123456) authenticated via OAuth 2.0..."
...
- ALWAYS include business_rule as simple category (Backend/Frontend/API/Database/UI/Security/etc)
- business_rule must be 1-2 words max, NEVER long sentences
- preconditions: 5-8 items, 50-150 chars each, moderately detailed with key specs
